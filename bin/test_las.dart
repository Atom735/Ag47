...
Future<void> main(List<String> args) async {
  ...
  for (final e in Directory('mappings').listSync(recursive: true, followLinks: false)) {
    if (e is File) {
    ...
    if (e is File && e.path.toLowerCase().endsWith('.las')) {
      ...
      try {
        e.readAsBytes().then((bytes) {
          ...
          for (final byte in bytes) {
            if (byte >= 0x80 && byteLast >= 0x80) {
              for (var i = 0; i < mappings.length; i++) {
                r[i] += freq_2letters(mappings[i][byteLast-0x80]+mappings[i][byte-0x80]);
              }
            }
            byteLast = byte;
          }
          ...
          final buffer = String.fromCharCodes(bytes.map((i) => i>=0x80?mappings[k][i-0x80].codeUnitAt(0):i));
